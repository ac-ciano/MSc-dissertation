...
if __name__ == '__main__':
    ...
    if thinking_enabled:
        results_path_val = os.path.join(output_dir, f'{model_name}_{style}_results-temp={thinking_temp}.csv')
        metrics_path_val = os.path.join(output_dir, f'{model_name}_{style}_metrics-temp={thinking_temp}.csv')
    else:
        results_path_val = os.path.join(output_dir, f'{model_name}_{style}_results-results.csv')
        metrics_path_val = os.path.join(output_dir, f'{model_name}_{style}_metrics-results.csv')
    

    run_gemini_inference(results_path=results_path_val,
                         metrics_path=metrics_path_val,
                         max_requests_per_minute=100) # Adjust RPM as needed