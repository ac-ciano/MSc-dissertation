...
base_output_model_dir = os.path.join(SCRIPT_DIR, "models", model_name + "_finetuned")
base_training_output_dir = os.path.join(SCRIPT_DIR, "training", model_name + "_training")
base_csv_metrics_filename = os.path.join(SCRIPT_DIR, "metrics", model_name + "_metrics")
base_logging_dir = os.path.join(SCRIPT_DIR, "logs", model_name + "_logs")
...
for fold_idx, (train_ids, val_ids) in enumerate(skf.split(np.zeros(len(full_dataset)), y)):
    ...
    training_arguments = TrainingArguments(
        ...
        save_total_limit=1,
        learning_rate=5e-5, # Increased from 1e-5. Consider also trying 1e-4 or 2e-4.
    )
