...
MODEL_ADAPTER_PATH = os.path.join(MODEL_PATH_ROOT, PREFIX + f"_FIRST_finetune", f"warmup320", f"models", f"roberta-large_finetuned_epoch_77")

# Path to the new data to evaluate and where to save the output
NEW_DATA_CSV_PATH = os.path.join(SCRIPT_DIR, '..', '..', 'data', 'posts_without_labels.csv')
OUTPUT_CSV_PATH = os.path.join(r"F:\VERO UTENTE\Desktop\Uni\dissertation\main\K-fold\large-cross-entropy--5e-5", f"data_fold{fold}\SECOND", f"SECOND_pseudol_{PREFIX}.csv")
...
def main():
    ...
    output_columns = ['index', 'post', 'predicted_label'] + [f'prob_{label}' for label in CLASS_LABELS]
