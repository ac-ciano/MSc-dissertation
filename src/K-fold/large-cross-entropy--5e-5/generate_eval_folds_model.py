...
PREFIX = "fold" + fold                 # Prefix used when saving the model in the training script
...
#MODEL_ADAPTER_PATH = os.path.join(MODEL_PATH_ROOT, f"models", f"roberta-large_finetuned_fold_" + fold)
MODEL_ADAPTER_PATH = r"F:\VERO UTENTE\Desktop\Uni\dissertation\main\K-fold\large-cross-entropy--5e-5\fold5_FIRST_finetune\warmup320\models\roberta-large_finetuned_epoch_77"

# Path to the new data to evaluate and where to save the output
EVAL_DATA = os.path.join(SCRIPT_DIR, f"data_fold" + fold, f"fold_" + fold + "_validation_split.csv")
...
def main():
    ...
    # --- Save Predictions ---
    output_filename = os.path.join(SCRIPT_DIR, "data_" + PREFIX, "FIRST", f"FIRST_{PREFIX}_validation_split_predictions.csv")
